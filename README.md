...
****Typed Entity** : An entity that is meaningful only if it co-occurs within a number or date or another entity. For example, "Revenue" is considered a Typed Entity only if it is found in a context where the actual dollar value is reported:
...
All functions must include a valid API key:
...
```
    dic = Dictionary(api_key='1234567')
```
...
After that we can create our Dictonary
```
    dic.create()
```
#### Response
...
#### Fetch Dictionary:
To fetch data from the dictionary
```
...
```
    tag = Tagging(api_key="1234")
```

#### Mining Content Files

First, upload all content files for tagging:
```
     tag.upload("file.pdf")
```
...
Then you can mine data via dictionaries. First you have to prepare additional options.
...
`vocabPath` (Required): The path to entity dictionary. Path is returned either after creation of a new dictionary or via listing of existing dictionaries.**There is no limit on the number of files and dictionaries that can be processed via tagging_files function**
```
...
`chunk`
(Optional, string) can be `SENTENCE` or `PARAGRAPH` or `NONE`. This will result in splitting of data into semantic chunks before processing. For example, this allow user to split the content of an article in semantic sentences and apply entity dictionaries at sentence level.

`excludeUttWithoutEntities`
(Optional, boolean) if `true` the output will only includes chunks that have at least one label from the input dictionaries.
...
Results can also be exported in XLSX format:
...
Both classes Dictionary and Tagging have a method clear to delete all used variable.
```
    dic.clear()
    tag.clear()
```
